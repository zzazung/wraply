...
async function runBuild(job) {
  ...
  return new Promise(async resolve => {
    ...
    if (!base?.appName) {
      throw new Error("Invalid settings: base.appName missing");
    }
    ...
    const packageName =
      target.config?.packageName ||
      base.packageName;   // fallback
    const safeName = `${(packageName || "app").replace(/\./g, "_")}_${jobId}`;
    ...
    try {
      ...
      if (platform === "android") {
        ...
        try {
          ...
          publishLog(jobId, tenantId,
            `[fatal] ios patch failed: ${err.message}`
          );
