...
router.get("/:safeName", async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      "SELECT id, safe_name, package_name, mode, key_alias, keystore_sha256, created_at, updated_at FROM android_signing_keys WHERE safe_name=?",
      [safeName]
    );
    const row = rows[0];
    res.json({
...
router.post("/:safeName/generate", async (req, res) => {
  try {
    ...
    const row = await ensureSigningKey(pool, { safeName, packageName, mode: "managed" });
    res.json({ success: true, id: row.id, mode: row.mode });
  } catch (e) {
...
router.post("/:safeName/upload", upload.single("file"), async (req, res) => {
  try {
    const safeName = req.params.safeName;
    const { packageName, alias, storePassword, keyPassword } = req.body || {};

    if (!req.file) return res.status(400).json({ error: "file required" });
    if (!packageName || !alias || !storePassword || !keyPassword) {
      return res.status(400).json({ error: "packageName/alias/storePassword/keyPassword required" });
    }
